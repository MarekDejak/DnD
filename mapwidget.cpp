...
MapWidget::MapWidget(QWidget *parent) : QWidget(parent)
{
    ...
    buttonPoint->setStyleSheet("color: rgb(0, 0, 0);font: 75 9pt ""Berlin Sans FB""; background-color: rgb(150, 150, 150);");
...
void MapWidget::onPrzyciskClicked(QString imie){
    if(!m_openCards.contains(imie)){ // otwiera sie tylko jesli nie jest juz otwarte
        ...
        connect(card, &QDialog::finished, this, [=](){m_openCards.removeOne(imie); setPtaszki(card->getPtaszki());});
